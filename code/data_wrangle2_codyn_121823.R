...
save(list=list_o_dfs$df,file=here("results","calculated_dfs_update2")) # save them (notice that objects are accessed as strings)
rm(list = ls())

#This will load the necessary objects that were created in the commented out script above
load(file=here("results","calculated_dfs_update2"))
upload1_ls = ls()
...
#   left_join(turnover_df) %>% 
#   left_join(stability_df) %>% 
#   left_join(rank_shift_df) 
...
# save(list=list_o_dfs2$df,file=here("results","calculated_dfs_12_19")) # joins were not working properly it should be fixed now
# rm(list = ls())

#This will load the necessary objects that were created in the commented out script above
load(file=here("results","calculated_dfs_12_19"))
upload2_ls <- ls()
...
rate_change_pa_df <- rate_change_pa_df %>% 
  mutate(Species = "PresentAbsent")

rate_change_df <- rate_change_df %>% 
  mutate(Species = "Cover_pct")


rate_change_interval_pa_df <- rate_change_interval_pa_df %>% 
  mutate(Species = "PresentAbsent")

rate_change_interval_df <- rate_change_interval_df %>% 
  mutate(Species = "Cover_pct")
  

names(rate_change_pa_df)
...
#another update
#add years between sampling dates

# Update 4 - add years between sampling dates  ----------------------------


#create filler of sample years
#Right now there are enough spaces for 20 year
sampleyear_filler <- paste0("SampleYear_", seq(1:20)) %>% 
  as.data.frame() %>% 
  mutate(row = row_number()) %>%
  rename(SY = ".") %>% 
  pivot_wider(names_from = SY, values_from = row) %>% 
  mutate_all(., ~NA)

species_park_list = names(species_park)
park_years_wide <- data.frame() 
# year_filler <- paste0("SampleYear_",seq(1:50))

for (i in 1:length(species_park_list)){ #run a loop over the dataframes in the list
  
  #Remove extra columns and rows to be just the identifier columns, Replicate, EventYear, etc.
  #Add count for the year number for that particular quadrat
  panel_df <- species_park[[i]] %>%
    ungroup() %>% 
    dplyr::select(Replicate, Park, EcoSite, Park_Ecosite, EventPanel, Plot,Transect, Quadrat, EventYear) %>% #select individual identifiable variables
    distinct() %>% #remove extra rows
    group_by(Replicate, Park, EcoSite, Park_Ecosite, EventPanel, Plot,Transect, Quadrat) %>% #group by everything but year
    arrange(Park_Ecosite, EventPanel, Plot, Transect, Quadrat, EventYear) %>% #sort by evertying including year
    mutate(Replicate.count = rleid(EventYear)) %>%  #add consecutive numbers for each quadrat
    #pivot the data frame wide and make sure years are numeric
    pivot_wider(names_from=Replicate.count, values_from = EventYear, names_prefix = "SampleYear_") %>% #pivot wider by sample year 
    ungroup() %>% 
    mutate_at(vars(contains("SampleYear_")), as.numeric) #make sure years are numeric
  
  #delete after saving 1/8/24 , I don't think I need this.
  # panel_years_columns <- panel_df %>% 
  #   dplyr::select(starts_with("SampleYear_")) %>% 
  #   names()
  
  panel_df <- add_column(panel_df, !!!sampleyear_filler[setdiff(names(sampleyear_filler), names(panel_df))]) %>% #adds columnsa SampleYear_1 through 20, so the dataframes will combine
    mutate(row = row_number())
  
  #create columns that subtract all the years
  panel_df_yeardiff<- panel_df %>% 
    dplyr::select(starts_with("SampleYear_")) %>% 
    mutate(cur_data()[-1] - cur_data()[-ncol(.)]) %>% #create columns that subtract year 1 from year 2
    mutate(row = row_number()) %>% #add row number to bind back to other dataframe
    rename_with(~str_replace(.,'SampleYear_', 'diff_SampleYear_'))
  
  panel_df_allcols <- panel_df %>% 
    left_join(panel_df_yeardiff) %>% 
    dplyr::select(-diff_SampleYear_1)
  
  park_years_wide <- rbind(park_years_wide, panel_df_allcols)
  
  
  
}

park_years_wide <- park_years_wide%>% 
  janitor::remove_empty(., which = "cols")


park_years_long <-  park_years_wide %>% 
  pivot_longer(., cols = diff_SampleYear_2:diff_SampleYear_8) %>% 
  rename(years_diff = value, diff_SampleYear_name = name) 
...
setdiff(a, upload3_ls) 

upload4_ls = c(upload3_ls, "park_years_long", "park_years_wide")

save(list=upload4_ls,file=here("results","upload4")) # save them (notice that objects are accessed as strings)
rm(list = ls())
