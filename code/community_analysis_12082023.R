...
#data file with richness and other diversity indices calculated
#note: this will need to be verified, need to check what was done with event panel and some other thing that were corrected here
community <- read.csv(here("data","community.csv"), header = T) %>% #read data file
...
results_df <- data.frame() # make an empty dataframe for results
...
rich_plot_ecosite1 = community_ecosite %>% 
  ...
  geom_smooth(aes(x = SampleYear, y = S.obs, color = Park, linetype = EcoSite),method = "glm", se=F, linewidth=1.25 )+
...
rich_plot_ecosite2 = community_ecosite %>% 
  ...
  geom_smooth(aes(x = SampleYear, y = S.obs, color = Park_Ecosite),method = "glm", se=F, linewidth=1.25 )+
  #scale_color_manual(values = c("blue", "orange"))+
  facet_wrap(~Park)+
...
rich_plot_noecosite = community_noecosite %>% 
  ggplot(aes(x = SampleYear, y = S.obs)) + 
  geom_jitter(aes(color=Park), alpha =0.05)+
  geom_smooth(aes(x = SampleYear, y = S.obs, color = Park),method = "glm", se=F, linewidth=1.25 )+
  #scale_color_manual(values = c("blue", "orange"))+
  facet_wrap(~Park)+
...
stability_df %>% 
  ggplot(aes(x = community$S.obs, y =stability)) +
  geom_boxplot() +
  ylim(0,100)+
  facet_wrap(~Park)
