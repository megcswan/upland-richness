...
shapiro_test_community_ecosite <- data.frame()
...
for (i in 1:length(community_ecosite_park)){ #run a loop over the dataframes in the list
  ...
  p_value.shap = cbind(parks_ecosite[i], shapiro$p.value)
  
  shapiro_test_community_ecosite <- rbind(shapiro_test_community_ecosite, p_value.shap)
